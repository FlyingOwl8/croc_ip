...
public class DAO {
    ...
    private void createTable(String sqlCreateTable, String tableName) {
        try (Statement statement = connection.createStatement()) {
            ...
            throw new RuntimeException("Ошибка при создании таблицы " + tableName + ": " + e.getMessage());
        }
    }
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(sqlSelectDataByPassport)) {
            ...
            try (ResultSet result = preparedStatement.executeQuery()) {
                if (result.next()) {
                    ...
                    throw new IllegalStateException(String.format("Заявка по паспорту %s не найдена", passport));
                }
            } catch (JsonProcessingException e) {
                throw new RuntimeException(e);
            }
